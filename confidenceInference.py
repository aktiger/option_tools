...
def calPriceIntervalPositive(initPrice, N_sigma, volatility, leftTime, yearDay=252):
    return initPrice * np.exp(N_sigma * volatility * np.sqrt(leftTime / yearDay))


def calPriceIntervalNegative(initPrice, N_sigma, volatility, leftTime, yearDay=252):
    return initPrice * np.exp(-N_sigma * volatility * np.sqrt(leftTime / yearDay))


def fullIntervalWithNSigma(initPrice, N_sigma, volatility, leftTime, yearDay=252):
...
def fullIntervalWithProb(initPrice, prob, volatility, leftTime, yearDay=252):
